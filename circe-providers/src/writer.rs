...
use crate::token_range::ScyllaTokenRangeProvider;

/// Reads from a [`ScyllaTokenRangeProvider`] and writes hive-partitioned Parquet files.
///
...
pub async fn write_hive_partitioned_parquet(
    ctx: &SessionContext,
    provider: Arc<ScyllaTokenRangeProvider>,
    output_url: &str,
    partition_columns: Vec<String>,
) -> Result<()> {
    let table_name = format!("_circe_{}_{}", provider.keyspace(), provider.table());
